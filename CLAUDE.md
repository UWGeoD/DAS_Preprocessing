...
This is a **Distributed Acoustic Sensing (DAS)** data processing and ML toolkit. The primary task is **denoising**; detection and weight estimation are secondary.
...
HDF5 Files → DAS.py (loading) → preprocessing.py (signal processing)
    ...
    → train.py --task denoising  →  predict.py --task denoising  (saves data/<dataset>/denoised/)
    → compute_metrics.py (SNR: raw vs pp vs denoised → results/metrics.csv)
    → eval_*.ipynb (load saved model + splits.json → visualize)
    [secondary] → train.py --task detection / weight
```

Training order matters: denoising must run first, then `predict.py --task denoising` generates `data/<dataset>/denoised/` before detection/weight training.
...
- `DASSampleDataset` — raw input → preprocessed target (for denoising training). Normalizes by subtracting per-channel mean from raw, then dividing both raw and clean by the centered raw's std (preserves the raw→clean ratio without DC offset inflating std). Supports optional augmentation (`augment=True`): channel-flip and time-reversal, each with p=0.5.
...
**[prepare_data.py](prepare_data.py)** — Data preparation CLI. Reads HDF5 files via `MulDAS`, adds `frame_end_bridge` via physics-based estimate, slides a fixed-length window over the DAS array, and writes `data/<dataset>/raw/sample_XXXXXX.npy` + `data/<dataset>/labels.csv`. The CSV includes a `signal_rects` column: per-window list of `[frame_start-3, frame_start+27]` time rectangles (global video frames, clipped to window bounds) marking vehicle-induced signal regions. Leftover `_tmp_*.npy` files from interrupted runs are cleaned up at start. Hyperparameters come from the active dataset config (`configs/datasets/*.yaml`).
...
**[train.py](train.py)** — Unified CLI entry point for all three training tasks. Loads YAML config from `configs/{task}.yaml`, merges dataset config (sensor params), applies CLI overrides, and saves `results/<task>/splits.json` and `results/<task>/losses.png` alongside the model checkpoint. Denoising training uses UNetV2, supports configurable loss via `configs/losses.yaml` (`mse`, `l1`, `gan_nce`), and supports `split_mode: temporal` (default) or `split_mode: random`. `overlap_gap` skips samples between splits to prevent data contamination when windows overlap.

**[predict.py](predict.py)** — Inference-only CLI. `--task denoising` runs the saved UNetV2 on all `data/<dataset>/raw/*.npy`, applies the same per-sample normalization used during training, and writes `data/<dataset>/denoised/denoised_sample_XXXXXX.npy`. Stale `denoised_*.npy` files from previous runs are cleared first. `--task detection/weight` runs inference on a single `--input` file.
...
**[models/unet.py](models/unet.py)** — UNet baseline: 3 encoder blocks → bottleneck → 3 decoder blocks with skip connections. Dropout2d(0.3) at bottleneck and first two decoder blocks. Uses bilinear upsampling. Not used by default — kept as a reference/baseline.

**[models/](models/)** — Task models: `detection_cnn.py` (`DASCountCNN`), `detection_transformer.py` (`DASCountTransformer` — count + vehicle type dual-head), `weight_cnn.py` (`DASWeightCNN` — absolute-value input for polarity-invariant weight regression). Also `discriminator.py` and `patchnce.py` — used only when `loss: gan_nce` is set in `denoising.yaml`.
...
**`config.py`** (gitignored) — set `ACTIVE_DATASET = "newville_nov2025"` to switch between datasets. Contains `load_dataset_config(name=None)` which loads `configs/datasets/{name}.yaml`.

**`configs/datasets/*.yaml`** — per-dataset config, authoritative for sensor/data params:
- File paths: `recording_dir`, `das_file`, `video_path`, `labeling_csv`
- Sensor: `vendor`, `fs_das`, `dx`, `channel_start`, `channel_end`
- Time alignment: `fps_video`, `frame_ref`, `sys_time_ref_hms`, `das_start_hms`
- Bridge geometry: `estimate_speed_mph`, `side_length_ft`, `bridge_length_m`
- Windowing: `window_length_s`, `stride_s`, `shuffle_samples`
- Output: `data_dir`, `out_dir`, `csv_out`

**`configs/{task}.yaml`** — per-task hyperparameters (model, training, preprocessing steps); CLI flags override YAML values.

**`configs/losses.yaml`** — loss function registry for denoising. Set `loss: <name>` in `denoising.yaml`. Available: `mse`, `l1`, `gan_nce` (GAN + PatchNCE contrastive loss).

### Training & Inference CLI

```bash
# Data preparation (run once per dataset; re-run if labels or config change)
python prepare_data.py                              # uses ACTIVE_DATASET from config.py
python prepare_data.py --dataset newville_nov2025   # explicit dataset

# Train (saves best_model.pt + splits.json to results/<task>/)
python train.py --task denoising
python train.py --task denoising --dataset newville_nov2025
# Override any YAML value via flags:
python train.py --task denoising --epochs 50 --lr 1e-5

# Inference
python predict.py --task denoising                           # → data/<dataset>/denoised/
...
4. **compute_metrics.py computes metrics only** — reads pre-generated files (raw `.npy`, denoised `.npy`), writes `results/metrics.csv`. No training, no inference.
...
8. **Config-driven** — all hyperparameters live in `configs/<task>.yaml` and `configs/datasets/<dataset>.yaml`. Never hardcode LR, epochs, batch size, or file paths in `.py` files.
